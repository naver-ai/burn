...
def train_svm(opts):
    ...
    print("Num task: {}".format(num_task))
    args_cls = []
    args_cost = []
    for cls in cls_list:
    ...
    #    pass
    #i = 0
    #for cls, cost, opts, feats, targets in zip(args_cls, args_cost, args_opts, args_features, args_targets):
    #  i += 1
    #  task(cls, cost, opts, feats, targets)
    #  print("{} th task finished".format(i))
    pool = ThreadPool(mp.cpu_count())
    results = pool.map(mp_helper, zip(args_cls, args_cost, args_opts, args_features, args_targets))
